...
idx = text.rfind('Siguiente Paso: Balance General')
if idx != -1:
    start = text.rfind('<div class="d-flex', 0, idx)
    if start != -1:
        html_section = '''
...
'''
        ...
        with open(r'c:\Users\VICTUS\Desktop\A.RISK ERM\templates\financial_planning\wizard.html', 'w', encoding='utf-8') as f:
            f.write(text)
        print('Inserted via robust fallback successfully')
    else:
        print('Div not found')
else:
    print('Siguiente paso not found')
