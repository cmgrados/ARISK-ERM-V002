                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        window.loadHistoricalPortfolio = function() {
                            ...
                            fetch("?plan_id=" + planId + "&dates=" + selectedDates)
                                .then(r => r.json())
                                .then(data => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const projStartYearInput = document.getElementById('0');
        const projYearsInput = document.getElementById('0');
        ...
        function lockStep6() {
            ...
            $.ajax({
                ...
                headers: { "X-CSRFToken": document.querySelector('[name=csrfmiddlewaretoken]') ? document.querySelector('[name=csrfmiddlewaretoken]').value : '0' },
                success: function(resp) {
                    Swal.close();
                    if(resp.status === 'success') {
                        isStep6Locked = true;
                    ...
                    if (response.status === 'success') {
                        ...
                        window.saveScenarioAdjustments = function(btn, vId, agencyId) {
                            ...
                            $.ajax({
                                ...
                                headers: {
                                    "X-CSRFToken": document.querySelector('[name=csrfmiddlewaretoken]') ? document.querySelector('[name=csrfmiddlewaretoken]').value : '0'
                                },
                                success: function(response) {
                                    if (response.status === 'success') {
                                        Swal.fire({
