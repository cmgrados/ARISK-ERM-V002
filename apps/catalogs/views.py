...
def test_integration_connection(request, pk):
    from .models import SystemIntegration
    integration = get_object_or_404(SystemIntegration, pk=pk)
    
    if not integration.api_key and integration.provider in ['gemini', 'openai', 'mistral']:
        return JsonResponse({'status': 'error', 'message': 'No se ha configurado una Clave API.'})
    ...
    try:
        if integration.provider == 'gemini':
            ...
            if response.text:
                return JsonResponse({'status': 'success', 'message': 'Conexión a Gemini establecida exitosamente.'})
            ...
            if res.status_code == 200:
                return JsonResponse({'status': 'success', 'message': 'Conexión a OpenAI establecida exitosamente.'})
            else:
                return JsonResponse({'status': 'error', 'message': f'Error OpenAI: {res.status_code} - {res.text}'})
                
        elif integration.provider == 'mistral':
            headers = {"Authorization": f"Bearer {integration.api_key}"}
            res = requests.get("https://api.mistral.ai/v1/models", headers=headers)
            if res.status_code == 200:
                return JsonResponse({'status': 'success', 'message': 'Conexión a Mistral establecida exitosamente.'})
            else:
                return JsonResponse({'status': 'error', 'message': f'Error Mistral: {res.status_code} - {res.text}'})
                
        else:
            return JsonResponse({'status': 'info', 'message': 'Prueba automática de conexión no disponible para este proveedor todavía.'})
