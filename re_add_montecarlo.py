...
# Find the save buttons container
match = re.search(r'<div class="d-flex justify-content-center mt-5 mb-4">[\s\S]*?Siguiente Paso: Balance General', text)

if match:
    text = text[:match.start()] + html_section + '\n' + text[match.start():]
    with open(r'c:\Users\VICTUS\Desktop\A.RISK ERM\templates\financial_planning\wizard.html', 'w', encoding='utf-8') as f:
        f.write(text)
    print("SUCCESS: Added montecarlo-section before Siguiente Paso buttons.")
else:
    print("ERROR: Could not find Siguiente Paso buttons.")
